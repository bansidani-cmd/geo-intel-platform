...
async function fetchJSON(url, fallback, timeoutMs = 10000) {
...
const SOLAR_SYSTEM_API =
  'http://localhost:8001/api/solar-system';
 ...
 *
 * while allowing the entire system to fit comfortably
 * inside the dashboard.
 */

const SOLAR_DISTANCE_OFFSET = 30;
...
function createSolarSystemBody(body) {
  const radius =
    SOLAR_SYSTEM_RADII[body.id] || 2.5;
...
function getSolarVisualPosition(
  body,
  sun,
) {
  const x =
...
function updateSolarSystemBodies(
  bodies,
) {
  solarSystemBodies =
  ...
  if (!sun) {
    console.warn(
      'Solar System: Sun not available.',
    );
  ...
  for (
    const body of solarSystemBodies
  ) {
    activeIds.add(
      body.id,
    );
...
function showJourneyView(
  ...
  /*
   * Disable automatic Earth rotation
   * while the journey controller operates.
   */
  ...
  /*
   * Allow external journey controller.
   */
...
window.returnToEarthView =
  returnToEarthView;

window.focusSolarSystem =
  focusSolarSystem;
...
window.solarSystemDebug =
  function () {
    ...
    console.log(
      'Bodies:',
      solarSystemBodies.length,
    );

    console.log(
      'Objects:',
      solarSystemObjects.size,
    );
...
function showPanel(data) {
  ...
  rows.push([
    'Type',
    getShipTypeLabel(data.shipType),
  ]);

  rows.push([
    'Status',
    getNavigationStatusLabel(data.navigationStatus),
  ]);
  ...
  rows.push([
    'Latitude',
    safeNumber(data.lat)?.toFixed(4) ?? 'Unknown',
  ]);

  rows.push([
    'Longitude',
    safeNumber(data.lng)?.toFixed(4) ?? 'Unknown',
  ]);
